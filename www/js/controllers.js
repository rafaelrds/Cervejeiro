...
.controller('AppCtrl', function($scope, $state, $ionicModal, $ionicPopover, $timeout, AuthService, FirebaseService, $q, $ionicPopup) {
    ...
    function abrePopupLembrete() {
        ...
        var myPopup = $ionicPopup.show({
            template: '<input type="checkbox" ng-model="data.naoLembrar">Não mostre isto novamente<br><br>' + 'So exiba lembrete a partir das <input type="number" min="0" max="23" ng-model="data.horaLembrete"><br><br>' + 'Você não registrou nenhum Tempo Investido ontem, gostaria de registrar agora?',
    ...
    (function main() {
        if (!AuthService.isUsuarioUndefined()) {
            ...
            lembretesPromise.then(function(info) {
                ...
                var lembrete = { lembrar: true, delay: 0 };
